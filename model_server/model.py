...
# Detailed model logs are disabled by default.
OWL_DEBUG = os.environ.get("OWL_DIFF_SEARCH_DEBUG", "").strip().lower() in ("1", "true", "yes", "on")
...
def encode_code(
    ...
    if report:
        # Model initialization can take longer than a batch. Report it as an
        # indeterminate phase instead of leaving an apparently frozen bar.
        progress.start("Loading embedding model", total)
            ...
            for batch_index, start_idx in enumerate(range(0, total, batch_size), start=1):
                ...
                if report:
                    progress.update(
                        start_idx,
                        total,
                        phase=f"Embedding batch {batch_index}/{batch_count}",
                    )
