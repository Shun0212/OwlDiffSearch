...
def snapshot() -> dict:
    ...
    now = time.time()
    end_time = now if snap.get("active") else (snap.get("updated_at") or now)
    elapsed = max(0.0, end_time - started) if started else 0.0
    snap["elapsed"] = elapsed
    snap["idle_for"] = max(0.0, now - (snap.get("updated_at") or now))
    # 残り時間の推定（現在の処理速度から外挿）
    eta = None
