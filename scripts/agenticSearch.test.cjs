...
function toolResponse(decision) {
  ...
  const name = action === 'finish' ? 'finish_search' : args.mode === 'keyword' ? 'keyword_search' : 'search_diff';
...
test('multiple native calls are executed sequentially within the shared budget and each receives a response', async () => {
  ...
  const fixture = setup([], [[hit('a')], [hit('b')]], {
    ...
    generate: async request => {
      ...
      assert.deepEqual(request.config.toolConfig.functionCallingConfig.allowedFunctionNames, ['finish_search']);
