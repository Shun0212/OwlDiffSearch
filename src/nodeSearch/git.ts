...
import { formatDiffRange, validateGitRef } from '../diffUtils';
        ...
        if (header && file && (added.length || removed.length)) {
            ...
            if (allowed) {
                ...
                units.push({
                    ...
                    lineno: added[0]?.[0] ?? Math.max(1, newStart), end_lineno: Math.max(1, newLine - 1),
...
export async function collectHunks(req: SearchRequest, signal?: AbortSignal): Promise<DiffUnit[]> {
    ...
    const recent = base ? 0 : Math.max(0, Math.min(10000, Math.floor(req.recent_commit_limit || 0)));
    if (base || head || recent) {
        const rev = recent ? head || 'HEAD' : base ? `${base}..${head || 'HEAD'}` : `HEAD..${head}`;
        const output = await git(root, ['log', '--encoding=UTF-8', '--no-show-signature', '-p', '--no-color',
            '--no-ext-diff', '--no-textconv', '--unified=3', '--root',
            ...(req.first_parent ? ['--first-parent', '--diff-merges=first-parent'] : []),
            ...(recent ? ['--date-order', `--max-count=${recent}`] : []),
...
export function buildCommitUnits(hunks: DiffUnit[]): DiffUnit[] {
    ...
    for (const commit of commits.values()) {
        ...
        for (const [file, group] of files) {
            ...
            units.push({ ...group[0], name: title, function_name: title, symbol_kind: 'diff_commit',
